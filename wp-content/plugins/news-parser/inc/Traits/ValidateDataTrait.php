...
trait ValidateDataTrait
{
    ...
    /** 
     * Validate structure of input post options.
     * Structure:['aiProviders','featuredImage','postTitle','postBody']
     * 
     * @param array $ai_options
     * @return true|\WP_Error
     */
    public function validateAIOptions($ai_options)
    {
        $ai_option_should_have_keys=array(
            'aiProviders',
            'featuredImage',
            'postTitle',
            'postBody'
        ...
        return $this->checkArrayKeys($ai_option_should_have_keys, $ai_options);
    }
    /**
    ...
    public function validateTemplate($template)
    {
        ...
        return $this->validateExtraOptions($template['extraOptions'])&&
            $this->validateHTMLTemplate($template['template'])&&
            $this->validateAIOptions($template['aiOptions']);
    }
    /**
     * Validate structure of input options for create post from parsed data.
     * 
     * @param array $options
     */
    public function validateOptions($options)
    {
        $options_should_have_keys=array(
            'extraOptions',
            'postOptions',
            'aiOptions'
        );
        if (!$this->checkArrayKeys($options_should_have_keys, $options)) {
            return false;
        }
        return $this->validateExtraOptions($options['extraOptions'])&&
            $this->validateAIOptions($options['aiOptions']);
    }
    public function validateParsedData($parsed_data)
    {
        $parsed_data_should_have_keys=array(
            'body',
            'title',
            'sourceUrl',
            'image'
        );
        if (!$this->checkArrayKeys($parsed_data_should_have_keys, $parsed_data)) {
            return false;
        }
        return true;
    }
