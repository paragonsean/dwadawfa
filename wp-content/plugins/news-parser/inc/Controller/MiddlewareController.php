...
class MiddlewareController {
    ...
    */
    ...
    protected function addFilter($action){
        ...
        if($this->has($action)) \add_filter($action,array($this,'apply_'.$action),10,2);
    }
    /**
    ...
    public function __call($method,$args){
        ...
        if(strpos($method,'apply_')===0){
            ...
            return $this->apply($action,$args[0],$args[1]);
        }
    }
    /**
     ...
     * 
     * @param string $action action name.
     * @param any $filtered_value value that should be passed to modifiers as filtered value
     * @param array $args value that should be passed to modifiers as additional args
     *   
     */
    protected function apply($action,$filtered_value,$args){
        if (!$this->has($action)) return $filtered_value;
        $result=array_reduce($this->middleware[$action],function($acc,$middleware)use($args){
            ...
            return \call_user_func_array($middleware,[$acc,$args]);
        },$filtered_value);
        return $result;
    }
}