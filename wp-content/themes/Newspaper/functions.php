...
//theme update notification disable
...
function gt_email_cron() {
    ...
    foreach($selected_categories as $cat): // loop start
    
    $category = get_category($cat);
      
    $q = str_replace(' ', '+', $category->name);
    // From URL to get webpage contents.
    $url = "https://gnews.io/api/v4/search?q=$q&country=us&topic=&expand=content&token=$access_key_id";
    //$url = "https://newsdata.io/api/1/news?apikey=pub_5072296e10645e1ecf7c8f7e126158a71e7b';
    
    wp_mail( 'test@gmail.com', 'Testing New Cron Event', $url );
    ...
    $news_data = json_decode($result);
    foreach($news_data->articles as $key => $item){
		...
		gksa_upload_from_url($item->image, $post_id);
	}
	
	endforeach; // loop end
}

function gksa_upload_from_url( $image_url, $attach_to_post = 0, $add_to_media = true ) {
